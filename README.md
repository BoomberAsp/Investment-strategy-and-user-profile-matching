...
2. **Word2Vec 深度学习路线（辅线）**：交易序列 Token 化 + 嵌入表示，用于交叉验证（Phase 3 实现）
...
│   │       └── word2vec.py              # 预留：Token + Word2Vec（Phase 3）
│   │
...
| **推荐策略** | Top-3 推荐 + 维度级归因 + 弹窗话术预览 |
| **匹配稳定性** | 多窗口推荐对比 + 趋势图 + 一致性结论 |
| **设置** | β 手动调整、数据导出/清除、后端切换 |
...
- **匹配引擎抽象**：通过 `MatchingBackend` 接口实现算法解耦，支持多后端切换
...
- [`report.md`](report.md) — 数据分析完整报告，含假设体系、公式推导、结果解读
- [`docs/APP_DESIGN.md`](docs/APP_DESIGN.md) — 网页应用详细设计文档
